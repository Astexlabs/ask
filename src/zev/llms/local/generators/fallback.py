    ...
    def generate(self, parsed: ParsedQuery) -> List[Command]:
        ...
        if parsed.file_types:
            ...
            commands.append(Command(
                command=f"# Try: zev 'find {parsed.file_types[0]} files'",
        ...
        if not commands:
            commands.append(Command(
                command="# Try: zev 'find python files from the last hour'",
                short_explanation="Example: find files by type and time",
                is_dangerous=False,
            ))
            commands.append(Command(
                command="# Try: zev 'search for TODO in js files'",
                short_explanation="Example: search file contents",
                is_dangerous=False,
            ))
            commands.append(Command(
                command="# Try: zev 'show disk usage' or 'list running processes'",
