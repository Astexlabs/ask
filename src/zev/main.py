...
from zev.config.tui import run_config_tui
from zev.constants import CONFIG_FILE_NAME, LLMProviders
...
def _ensure_default_local_config(config_path: Path) -> None:
    """Create a minimal .zevrc with LLM_PROVIDER=local so the app doesn't
...
def _show_local_banner() -> None:
    ...
    rprint(
        "\n[bold cyan]"
        "  ╭──────────────────────────────────────╮\n"
        "  │     zev · [yellow]Local Mode[/yellow] (offline)      │\n"
    ...
    if not config_path.exists():
        ...
        if is_setup:
            ...
            rprint(
                ...
                "Run [bold]zev --setup[/bold] or [bold]zev --config[/bold] to configure.[/dim]\n"
            )
